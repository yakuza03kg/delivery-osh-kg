...
@section('content')
    ...
    @if($canEdit)
        <form class="card form-card narrow-card" method="POST" action="{{ route('admin.api-usage.update') }}">
            ...
            <div class="card-heading">
                ...
                <span class="tag tag-orange">Супер-администратор</span>
            </div>
    ...
    @else
        <section class="card form-card narrow-card">
            <div class="card-heading">
                <div>
                    <h2>Синхронизация ограничена</h2>
                    <p>Просматривать остаток могут все администраторы. Изменять лимиты и значения синхронизации может только супер-администратор.</p>
                </div>
                <span class="tag tag-muted">Только просмотр</span>
            </div>
            <div class="notice notice-neutral">Обратитесь к супер-администратору, чтобы обновить лимиты по данным Platform Manager.</div>
        </section>
    @endif
@endsection
