...
@section('content')
    ...
    <form class="card form-card narrow-card" method="POST" action="{{ $user->exists ? route('admin.users.update', $user) : route('admin.users.store') }}">
        ...
        <div class="form-grid">
            ...
            <label class="field"><span>Роль</span><select name="role" required><option value="courier" {{ old('role', $user->role) === 'courier' ? 'selected' : '' }}>Курьер</option><option value="admin" {{ old('role', $user->role) === 'admin' ? 'selected' : '' }}>Администратор</option></select></label>
            <div></div>
